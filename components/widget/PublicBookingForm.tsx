      ...
      <div className="cb-scroll-shell">
        <form action="/api/bookings" method="POST" className="cb-fade-up space-y-8 px-6 pb-6 pt-6 md:px-10 md:pb-8 md:pt-10">
          ...
          <div className="pb-8 text-center" style={{ borderBottom: `1px solid ${softerBorderColor}` }}>
            ...
            <p className="mx-auto mt-5 max-w-2xl text-[17px] leading-8" style={{ color: mutedTextColor }}>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <p className="text-[18px] font-semibold" style={{ color: isSelected ? widget.text_color : subtleTextColor }}>
                          {service.name}
                        </p>
                        {service.description && (
          ...
          <section className="space-y-4">
            ...
            {!selectedServiceId ? (
              <InfoBox borderColor={softBorderColor} backgroundColor={softSurface} color={mutedTextColor}>
              ...
              <div className="rounded-[28px] border p-5" style={{ borderColor: softBorderColor, backgroundColor: 'transparent' }}>
                ...
                <div className="mb-3 grid grid-cols-7 gap-2">
                  {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                    <div key={day} className="py-2 text-center text-[11px] font-semibold uppercase tracking-[0.12em]" style={{ color: mutedTextColor }}>
                      {day}
                    </div>
                  ))}
                </div>
                  ...
                  {calendarDays.map((day) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isToday && !isSelected && (
                          <span className="pointer-events-none absolute inset-[3px] rounded-[16px] border" style={{ borderColor: accentBorder }} />
                        )}

                        {isAvailable && !isSelected && (
                          <span className="absolute bottom-1.5 left-1/2 h-1.5 w-1.5 -translate-x-1/2 rounded-full" style={{ backgroundColor: widget.accent_color }} />
                        )}
                      </button>
                    )
                  })}
                ...
                {selectedDate && (
                  <div className="mt-5 rounded-[22px] border px-4 py-3 text-sm" style={{ borderColor: accentBorder, backgroundColor: accentSoft, color: widget.text_color }}>
          ...
          {selectedDate && selectedTime && (
            <div className="rounded-[24px] border p-5" style={{ borderColor: accentBorder, backgroundColor: accentSoft }}>
              <p className="text-sm font-semibold" style={{ color: widget.text_color }}>
          ...
          <section className="grid gap-5 md:grid-cols-2">
            <WidgetInput label="Name" name="customer_name" placeholder="John Smith" required color={widget.text_color} borderColor={softBorderColor} backgroundColor={softSurface} />
            <WidgetInput label="Email" name="customer_email" type="email" placeholder="john@company.com" required color={widget.text_color} borderColor={softBorderColor} backgroundColor={softSurface} />
          </section>

          <WidgetInput label="Phone" name="customer_phone" placeholder="(555) 555-5555" color={widget.text_color} borderColor={softBorderColor} backgroundColor={softSurface} />
...
function InfoBox({
  ...
  return (
    <div className="rounded-[24px] border p-4 text-sm" style={{ borderColor, backgroundColor, color }}>
      {children}
    </div>
  )
}
...
function WidgetInput({
  ...
  return (
    <div>
      <label className="mb-2 block text-[15px] font-semibold" style={{ color }}>
        {label}
      </label>
      <input
        name={name}
...
function ClickBooksFooter({ borderColor }: { borderColor: string }) {
  return (
    <div
      className="sticky bottom-0 z-10 border-t px-6 py-3 backdrop-blur"
      style={{
        borderColor,
    ...
    >
      <div className="flex items-center justify-center gap-2">
        <span className="text-[11px] uppercase tracking-[0.18em] text-[var(--text-muted)]">Powered by</span>
        <img src={clickBooksLogo} alt="ClickBooks" className="h-16 w-auto object-contain opacity-95" />
      </div>
    </div>
  )
}

function WidgetStyles({ accentSoft, accentColor }: { accentSoft: string; accentColor: string }) {
  return (
    <style>{`
      html,
        body {
